...
y = data['hs_grad']
regression = MyLineReg(metric='r2',learning_rate=0.5,n_iter=300,l1_coef=0.15,l2_coef=0.15,sgd_sample=1.0)
regression.fit(X,y,5)
print(regression.get_best_score())
print(pd.concat([regression.predict(X),y],axis=1))
print(regression.get_coef())
print(max(data['poverty']))
print(regression.get_best_score())
